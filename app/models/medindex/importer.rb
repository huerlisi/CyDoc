module Medindex
  class Importer
    ...
    def self.path(import_class)
      case ENV['RAILS_ENV']
        when 'production'
          File.join(RAILS_ROOT, 'data', 'Medindex', "DownloadMedindex#{import_class.name.demodulize}_out.xml")
        when 'development', 'test'
          File.join(RAILS_ROOT, 'test', 'fixtures', 'medindex', "DownloadMedindex#{import_class.name.demodulize}_out.xml")
      end
    end
  end
end
