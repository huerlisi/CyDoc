class BookkeepingController < ApplicationController
  ...
  def report
    ...
    @started_work      = Session.find(:all, :include => :invoices, :conditions => ["duration_from <= ? AND (invoices.value_date > ? OR invoices.id IS NULL)", @value_date_end, @value_date_end]).to_a.sum(&:amount)
