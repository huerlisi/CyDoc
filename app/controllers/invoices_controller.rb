...
class InvoicesController < AuthorizedController
  ...
  def new
    ...
    begin
      @invoice.tiers.biller = Person.find(current_tenant.settings['invoices.defaults.biller_id'])
    rescue
    end
    begin
      @invoice.tiers.provider = Person.find(current_tenant.settings['invoices.defaults.provider_id'])
    rescue
    end
